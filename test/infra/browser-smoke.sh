...
docker run -d --rm --name "$name" --memory=256m --memory-swap=256m --cpus=1 \
  -e "APP_ORIGIN=$base_url" -p "127.0.0.1:${port}:3000" "$image" >/dev/null
...
LIFE_OS_E2E_URL="$base_url" pnpm exec playwright test tests/e2e/finance-smoke.spec.ts --workers=1
